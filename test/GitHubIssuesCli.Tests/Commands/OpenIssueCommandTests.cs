...
{
    public class OpenIssueCommandTests
    {
        ...
        private const int OpenedIssueNumber = 101;
        private const string ValidComment = "This is a comment";
        
        public OpenIssueCommandTests()
        {
            ...
            _issuesClient.Setup(client => client.Get(ValidOwner, ValidRepo, ValidIssueNumber))
                .Returns(Task.FromResult(GitHubModelFactory.CreateIssue(ValidOwner, ValidRepo, ValidIssueNumber, ItemState.Closed)));
            _issuesClient.Setup(client => client.Get(ValidOwner, ValidRepo, OpenedIssueNumber))
                .Returns(Task.FromResult(GitHubModelFactory.CreateIssue(ValidOwner, ValidRepo, OpenedIssueNumber, ItemState.Open)));
        ...
        public async Task ValidationException_ReportsError()
        {
            ...
            var command = new OpenIssueCommand(_gitHubClient.Object, _discoveryService.Object, _reporter.Object)
            {
        ...
        [Fact]
        public async Task OpenedIssue_ReportsWarning()
        {
            // Arrange
            var command = new OpenIssueCommand(_gitHubClient.Object, _discoveryService.Object, _reporter.Object)
            {
                Issue = $"{ValidOwner}/{ValidRepo}#{OpenedIssueNumber}"
            };
            ...
            // Assert
            _reporter.Verify(r => r.Warn(It.Is<string>(s => s == "Issue jerriep/github-issues-cli#101 is already open. No action taken.")), Times.Once());
        }

        [Fact]
        public async Task OpenedIssue_DoesNotUpdateIssue()
        {
            // Arrange
            var command = new OpenIssueCommand(_gitHubClient.Object, _discoveryService.Object, _reporter.Object)
            {
        ...
        [Fact]
        public async Task OpenedIssue_ReturnsError()
        {
            // Arrange
            var command = new OpenIssueCommand(_gitHubClient.Object, _discoveryService.Object, _reporter.Object)
            {
        ...
        [Fact]
        public async Task ClosedIssue_UpdatesIssue()
        {
            // Arrange
            var command = new OpenIssueCommand(_gitHubClient.Object, _discoveryService.Object, _reporter.Object)
            {
            ...
            // Assert
            _issuesClient.Verify(client => client.Update(ValidOwner, ValidRepo, ValidIssueNumber, It.Is<IssueUpdate>(update => update.State == ItemState.Open)), Times.Once());
        }
